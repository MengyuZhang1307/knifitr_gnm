...
data = data %>% 
  ...
  mutate(asc_res = map(count_asc, ~acs_inter(.x)), # detect t0 and interpolate aif between t0 and tmax
         ...
         dsc_pred = map2(dsc_mod,count_dsc,~pred_aif(.x,.y)), # get prediction after tmax
         pred = map2(asc_pred,dsc_pred,~rbind(.x,.y[-1,])), # combine predicted aif
...
for (i in 1:nrow(data)){
  ...
  scope_whole(data[i,],0,5) # plot aif and predicted aif
  plot_rsd(data[i,],0,90) # plot residuals and boxplot with median
}
