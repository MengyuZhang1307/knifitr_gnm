...
data = data %>% 
  ...
  mutate(count = map(count,~findtmax(.x)),
         count_asc = map(count, ~slice_asc(.x)),
         count_dsc = map(count, ~slice_exp(.x)))
```
...
data = data %>% 
  group_by(patient) %>% 
  mutate(asc_res = map(count_asc, ~acs_inter(.x)),
         count_asc = map(asc_res, ~.x$data), # add t0 to the data
         asc_mod = map(asc_res, ~.x$segmod), # interpolate data for ascending part
         ...
         dsc_mod = map(dsc_mod, ~.x$result),
         asc_pred = map(asc_res, ~.x$pred),
         dsc_pred = map2(dsc_mod,count_dsc,~pred_aif(.x,.y)),
         pred = map2(asc_pred,dsc_pred,~rbind(.x,.y[-1,])),
         rsd = map2(count, pred, ~add_residual(.x,.y))
         ) %>% 
  select(-asc_res)
...
for (i in 1:nrow(data)){
  par(mfrow=c(1,1))
  scope_whole(data[i,],0,5)
  plot_rsd(data[i,],0,90) # with median
}
